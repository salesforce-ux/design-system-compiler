...
const compile = (sassPath, options={ wrapper: '' }, done) =>
  [chooseScopedOrNot(options.wrapper)]
  .map(fileName => path.join(sassPath, fileName))
  .map(filePath => fs.readFileSync(filePath, 'utf-8'))
  .map(contents => options.wrapper ? wrapContents(contents, options.wrapper) : contents)
